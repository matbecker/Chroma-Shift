...
public class Bouncer : Enemy {
	...
	protected override void Update ()
	{
		...
		{
			if (!spinning && rayDown.collider.gameObject == target)
			{
		...
		var rayLeft = Physics2D.Raycast(new Vector2(transform.position.x - sprite.bounds.extents.x - 0.05f, transform.position.y), Vector2.left, 0.1f);
		var rayRight = Physics2D.Raycast(new Vector2(transform.position.x + sprite.bounds.extents.x + 0.05f, transform.position.y), Vector2.right, 0.1f);
